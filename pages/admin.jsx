...
class AdminPage extends React.Component {
    ...
    componentDidMount() {
        ...
        fetcher("/api/user").then((res) => {
            if (res?.isLoggedIn) {
                selfthis.setState({isLoginFetched: true, isLoggedIn: true});
                getAllChannelsAsync("", 1, setLoadData).then((res) => {
                    ...
                    selfthis.callbackGroupSets(configuredCallback);
                });
            } else {
                console.info("Not logged in, redirecting...");
                Router.push("/login");
                selfthis.setState({isLoginFetched: true, isLoggedIn: false});
            }
        }).catch((err) => {
            selfthis.setState({isLoginFetched: true, isLoggedIn: false});
        })
    ...
    render() {
        const { isLoginFetched, isLoggedIn, isLoading, loadedData, current, max } = this.state;
        ...
        if (!isLoginFetched) {
            return (
                <>
                    <Head>
                        <HeaderDefault />
                        <title>Admin Page :: VTuber API</title>
                        <SEOMetaTags title="Admin Page" description="Restricted Access!" url="https://vtuber.ihateani.me/admin" />
                        <HeaderPrefetch />
                    </Head>
                    <main />
                </>
